<template>
  <div class="max-w-4xl mx-auto p-6">
    ...
    <ModalPago
        :open="mostrarPago"
        :cita-id="pagoInfo.citaId"
        :concepto="pagoInfo.concepto"
        :amount="pagoInfo.amount"
        :payment-url="pagoInfo.paymentUrl"
        :token="pagoInfo.token"
        @close="mostrarPago = false"
        @later="() => { mostrarPago = false; router.push('/cliente/mis-citas'); }"
    />
  </div>
</template>
...
const pagoInfo = ref({
  citaId: null,
  concepto: "Cita en Clínica de Fisioterapia",
  amount: 0,
  paymentUrl: "",
  token: "",
});
...
const confirmarReserva = async () => {
  ...
  try {
    ...
    // 1) Reservamos la cita (backend) y obtenemos su ID
    const citaCreada = await citaService.reservarCita(payload);

    // 2) Iniciamos el pago en el TPV (backend) y recibimos token + paymentUrl
    const respPago = await pagoService.iniciarPago(citaCreada.id);

    // 3) Mostramos una "ventanita" (modal) con el resumen y el botón para abrir el TPV
    const concepto = citaCreada?.servicio?.nombre
        ? `Cita: ${citaCreada.servicio.nombre}`
        : "Cita en Clínica de Fisioterapia";

    const amount =
        citaCreada?.precioPagado ??
        citaCreada?.servicio?.precio ??
        reserva.value?.servicio?.precio ??
        0;
    ...
    pagoInfo.value = {
      ...
      concepto,
      amount,
      paymentUrl: respPago?.paymentUrl || "",
      token: respPago?.token || respPago?.transactionId || "",
    };
